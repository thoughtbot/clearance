...
Clearance is based on the same conventions and tools as "Suspenders":http://github.com/thoughtbot/suspenders/tree/master Thus if you use it, you may already have some configuration mentioned below in place.
...
You may already have some of these files. Don't worry. You'll be asked if you want to override them.

h2. Modules

Clearance works by mixing behavior into tests, controllers, and models. For any file that you do not want to override, include the corresponding Clearance module. They are namespaced exactly like the directory structure of a Rails app.
...
h2. Usage: actions that require the user to be logged in
...
h2. Hooks and overrides: return_to parameter
...
h2. Hooks and overrides: url_after_create, url_after_destroy
    
Actions that redirect (create and destroy) in Clearance controllers are customizable. If you want to redirect a user to a specific route after signing in, override the "url_after_create" method in the SessionsController:
